...
def draw_bbox(i, image, cropped_image, bbox, gt_bbox, text, 
    ...
    file_path = os.path.join(output_path, "{}_ori_imag_with_crop_box.jpg".format(i))
...
def dynamic_boxes(bbox, image, box_size=512, num_candidates=4):
    ...
    for _ in range(num_candidates):
        ...
        new_x_min = random.randint(0, max(0,box_size - box_w))
        # print('new_x_min',new_x_min)
        new_y_min = random.randint(0, max(0,box_size - box_h))
        # print('new_y_min',new_y_min)
        # 根据新旧坐标反推crop图左上角在原图的位置
        crop_x_min = x_min - new_x_min
        crop_y_min = y_min - new_y_min

        # 限制crop区域不超出原图边界
        crop_x_min = max(0, min(crop_x_min, img_w - box_size))
        crop_y_min = max(0, min(crop_y_min, img_h - box_size))

        crop_x_max = crop_x_min + box_size
        crop_y_max = crop_y_min + box_size
        boxes.append((crop_x_min, crop_y_min, crop_x_max, crop_y_max))
...
def main():
    ...
    for anno in tqdm.tqdm(annotations):
        ...
        for i in range(len(region_list)):
            ...
            # 比较gtbox和restore box 选择最好的 记录bbox作为region
            iou = compute_iou(restore_bbox, gt_bbox)
            print(i,'个iou：',iou)
            if iou >= max_iou:
                max_iou = iou
                best_region = bbox
            ...
            #     draw_bbox(i,image_ori, cropped_image, bbox, gt_bbox, 
            #             input_question,
            #             args.save_path, image_name, question_type,
            #             hr_bbox, restore_bbox)
            iter_save = iter_save + 1
            # print('==================',iter_save)
            logger.info("best_region:{}".format(best_region)) # bbox 坐标保存即可 原图分辨率下的predicted region
        best_region_list = [int(x) for x in best_region]  # tuple 转 list，并转为原生 int
        anno["best_region"] = best_region_list
        # draw_bbox(6,image_ori, cropped_image, best_region, gt_bbox, 
        #                 input_question,
        #                 args.save_path, image_name, question_type,
        #                 hr_bbox, restore_bbox)

...
from verl.utils.torch_functional import get_constant_schedule_with_warmup, AnyPrecisionAdamW
...
class FSDPWorker(Worker):
    def __init__(
        ...
        self._is_rollout = self.role in ["rollout", "actor_rollout", "actor_rollout_ref"] # yjz add actor for rollout
    ...
    def _build_rollout(self) -> None:
        ...
        print('self.config.actor.model.model_path',self.config.actor.model.model_path)
        print('self.config.rollout',self.config.rollout)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_sequences_lr(self, prompts: DataProto):
        assert self._is_rollout
        ...
        with self.rollout_sharding_manager:
            ...
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            output = self.rollout.generate_sequences_lr(prompts=prompts)
            log_gpu_memory_usage("After rollout generation")

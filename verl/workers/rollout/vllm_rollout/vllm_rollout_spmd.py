...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self.config.n > 1 and do_sample:
            ...
            if "id" in non_tensor_batch.keys():
                non_tensor_batch["id"] = _repeat_interleave(non_tensor_batch["id"], self.config.n)
                non_tensor_batch["problem"] = _repeat_interleave(non_tensor_batch["problem"], self.config.n)
    ...
    def generate_sequences_lr(self, prompts: DataProto, **kwargs) -> DataProto:
        # left-padded attention_mask
        batch_tensor = prompts.batch
        input_ids: torch.Tensor = prompts.batch["input_ids"]  # (bs, prompt_length)
        attention_mask: torch.Tensor = prompts.batch["attention_mask"]
        position_ids: torch.Tensor = prompts.batch["position_ids"]
        eos_token_id: int = prompts.meta_info["eos_token_id"]

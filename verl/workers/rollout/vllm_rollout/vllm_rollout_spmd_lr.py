...
class vLLMRolloutLR(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if "id" in prompts.batch.keys():
            id: List = prompts.batch["id"]
            problem: List = prompts.batch["problem"]
        ...
        if self.config.n > 1 and do_sample:
            ...
            if "id" in prompts.batch.keys():
                id = _repeat_interleave(id, self.config.n)
                problem = _repeat_interleave(problem, self.config.n)

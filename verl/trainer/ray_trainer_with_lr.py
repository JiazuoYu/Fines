...
class RayPPOTrainer_with_LR:
    ...
    def _create_dataloader(self):
        self.train_dataset = RLHFDataset(
            ...
            old_prompt=self.config.data.old_prompt,
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            ...
            print("Init LR actor roll out !!")
            resource_pool = self.resource_pool_manager.get_resource_pool(Role.LRActorRollout)
            lr_actor_rollout_cls = RayClassWithInitArgs(
                cls=self.role_worker_mapping[Role.LRActorRollout], config=self.config.worker, role="actor"
            )
            self.resource_pool_to_cls[resource_pool]["actor"] = lr_actor_rollout_cls
        else:
            raise NotImplementedError
        ...
        self.lr_actor_rollout_wg: FSDPWorker = all_wg["actor"]
        self.lr_actor_rollout_wg.init_model()
    ...
    def _load_checkpoint(self):
        if self.config.trainer.load_checkpoint_path is None and self.config.trainer.resume_mode == "disbale":
            return 0
        ...
        for _ in range(self.config.trainer.total_episodes):
            for batch_dict in self.train_dataloader:
                ...
                if self.global_steps >= self.training_steps:
                    break
                # load_checkpoint非空时判断 小于保存step时continual 以跳过训练的step （yjz）
                metrics = {}
                timing_raw = {}
                ...
                if "pixel_values" in batch.non_tensor_batch.keys():
                    gen_batch = batch.pop(
                        batch_keys=["input_ids", "attention_mask", "position_ids"],
                        non_tensor_batch_keys=["pixel_values", "image_grid_thw", "raw_prompt_ids", "images", "id", "problem"],
                    )
                else:
                    gen_batch = batch.pop(
                        batch_keys=["input_ids", "attention_mask", "position_ids"],
                        non_tensor_batch_keys=["raw_prompt_ids", "id", "problem"],
                    )
                ...
                with _timer("step", timing_raw):
                    ...
                    self.train_dataset.update_batch_for_lr(batch)
                    
                    lr_gen_batch_output = self.lr_actor_rollout_wg.generate_sequences_lr(batch)
                    ...
                    #     reward_tensor, all_scores = self.reward_fn(batch, True)

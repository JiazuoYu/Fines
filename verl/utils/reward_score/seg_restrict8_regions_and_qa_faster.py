...
def seg_iou_reward(predict_str: str, ground_truth: str) -> float:
    ...
    try:
        ...
        if gt_match:
            gt_bbox = [
                int(gt_match.group(1)),
                int(gt_match.group(2)),
                int(gt_match.group(3)),
                int(gt_match.group(4)),
            ]
            gt_region = get_region_with_gt_box(gt_bbox)
        ...
        if json_match:
            ...
            if bbox_key and len(data[bbox_key]) == 4:
                ...
                if iou(content_bbox, gt_region) > 0.9:  # change
                    print(
                        "--------------------------------box iou 1.0-------------------------------",
                        "content_bbox{}".format(content_bbox),
                        "gt_region{}".format(gt_region),
                    )
                    return 1.0
    except Exception:
        pass
...
def seg_box_l1_reward(predict_str: str, ground_truth: str) -> float:
    ...
    try:
        ...
        gt_box_pattern = r"<box>\((\d+),(\d+)\),\((\d+),(\d+)\)</box>"
        gt_region_pattern = (
            r"<best_region>\((\d+),(\d+)\),\((\d+),(\d+)\)</best_region>"
        )
        gt_match = re.search(gt_box_pattern, ground_truth)
        # gt_region_pattern = r'<best_region>\((\d+),(\d+)\),\((\d+),(\d+)\)</best_region>'
        # gt_match_region = re.search(gt_region_pattern, ground_truth)
...
def seg_strict_compute_score8_region_qa_faster(predict_str: str, ground_truth: str) -> float:  # todo 传个type进来

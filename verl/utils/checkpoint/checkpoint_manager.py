...
def find_max_global_number(root_folder):
    max_number = -1 # /15324359926/seg/HD_Reasoning/Seg-Zero/workdir/resume_test_2/global_step_80
    pattern = re.compile(r'^global_step_(\d+)$')  # 正则匹配 global_ 后面的数字
...
def clean_old_ckpts(ckpt_base_path, current_step_folder):
    # 清除旧的 global_step_* 文件夹
    for name in os.listdir(ckpt_base_path):
        if re.match(r"^global_step_\d+$", name) and name != current_step_folder:
            old_ckpt_path = os.path.join(ckpt_base_path, name)
            print("Cleaning old ckpt files:{}".format(old_ckpt_path))
...
def find_latest_ckpt_path(path, directory_format="global_step_{}"):
    ...
    iteration = find_max_global_number(path)
